...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        applied_func = lambda expr: scheme_eval(expr, env)
        operand = rest.map(applied_func)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        name=target
        lambda_functs = Pair(name.rest, expressions.rest)
        new_procedure = do_lambda_form(lambda_functs, env)
        Frame.define(env, name.first, new_procedure)
        return name.first
        # END PROBLEM 9
    else:
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 6
    special_form = expressions.first
    return special_form
    # END PROBLEM 6
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    t_or_f = {}
    t_or_f.update( {'#t' : True} )
    t_or_f.update( {'#f' : False} )
    if expressions == nil:
        return t_or_f['#t']
    subex_eval_funct = lambda sub_expression: scheme_eval(sub_expression, env)
    last_true_value = t_or_f['#t']
    for num_expr in range(len(expressions)):
        subex=subex_eval_funct(expressions.first)
        if is_true_primitive(subex) and subex != nil:
            last_true_value = subex
        if is_false_primitive(subex) or subex == nil:
            return t_or_f['#f']
        else:
            expressions = expressions.rest
    return last_true_value
    # END PROBLEM 12
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            ...
            t_or_f = {}
            t_or_f.update( {'#t' : True} )
            else_expr = clause.first
            if clause.rest == nil:
                return test
            elif clause.rest != nil:
                return eval_all(clause.rest, env)
            elif clause.first != nil and else_expr.first != nil:
                return eval_all(clause.first, env)
            elif clause.first == nil:
                return t_or_f['#t']
            else:
                return None
            # END PROBLEM 13
